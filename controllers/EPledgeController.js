const { EPledge, UserPledge, EAward } = require("../models/EPledge");
...
const createPledge = async (req, res) => {
  try {
    ...
    const newPledge = await EPledge.create({ pledgeText, gift, points, imageUrl });
...
const getAllPledges = async (req, res) => {
  try {
    const pledges = await EPledge.find().sort({ createdAt: 1 });
    ...
    if (userId) {
      const userPledges = await UserPledge.find({ user: userId });
...
const getPledgeById = async (req, res) => {
  try {
    const pledge = await EPledge.findById(req.params.id);
...
const updatePledge = async (req, res) => {
  try {
    ...
    const updated = await EPledge.findByIdAndUpdate(req.params.id, updateData, { new: true });
...
const deletePledge = async (req, res) => {
  try {
    ...
    await UserPledge.deleteMany({ pledge: pledge._id });
...
const completePledge = async (req, res) => {
  try {
    ...
    const pledge = await EPledge.findById(pledgeId);
    if (!pledge) return res.status(404).json({ success: false, message: "Pledge not found" });

    let userPledge = await UserPledge.findOne({ user: userId, pledge: pledgeId });
    if (!userPledge) userPledge = new UserPledge({ user: userId, pledge: pledgeId });

    if (userPledge.isCompleted) return res.json({ success: true, message: "Pledge already completed", data: userPledge });
...
// ------------------- GET USER COMPLETED PLEDGES -------------------
const getUserCompletedPledges = async (req, res) => {
  try {
    const userId = req.user.id;

    const completed = await UserPledge.find({ user: userId, isCompleted: true }).populate({
      path: "pledge",
    ...
    const formatted = completed.map((item) => ({
      ...
      pledge: {
        ...
        imageUrl: item.pledge?.imageUrl ? `${BASE_URL}${item.pledge.imageUrl}` : `${BASE_URL}/uploads/default.jpg`,
      },
    }));
...
const createAward = async (req, res) => {
  try {
    ...
    const award = await EAward.create({ title, description, icon });
...
const getAwards = async (req, res) => {
  try {
    const awards = await EAward.find().sort({ createdAt: 1 });
...
const updateAward = async (req, res) => {
  try {
    const updated = await EAward.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
const deleteAward = async (req, res) => {
  try {
    const deleted = await EAward.findByIdAndDelete(req.params.id);
