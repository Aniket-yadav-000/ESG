const { SPledge, UserSPledge, SAward } = require("../models/SPledge");
...
const createPledge = async (req, res) => {
  try {
    ...
    const newPledge = await SPledge.create({ pledgeText, gift, points, imageUrl });
...
const getAllPledges = async (req, res) => {
  try {
    const pledges = await SPledge.find().sort({ createdAt: 1 });
    ...
    if (userId) {
      const userPledges = await UserSPledge.find({ user: userId });
...
const getPledgeById = async (req, res) => {
  try {
    const pledge = await SPledge.findById(req.params.id);
...
const updatePledge = async (req, res) => {
  try {
    ...
    const updated = await SPledge.findByIdAndUpdate(req.params.id, updateData, { new: true });
...
const deletePledge = async (req, res) => {
  try {
    ...
    await UserSPledge.deleteMany({ pledge: pledge._id });
...
const completePledge = async (req, res) => {
  try {
    ...
    const pledge = await SPledge.findById(pledgeId);
    if (!pledge) return res.status(404).json({ success: false, message: "Pledge not found" });

    let userPledge = await UserSPledge.findOne({ user: userId, pledge: pledgeId });
    if (!userPledge) userPledge = new UserSPledge({ user: userId, pledge: pledgeId });
...
const getUserCompletedPledges = async (req, res) => {
  try {
    ...
    const completed = await UserSPledge.find({ user: userId, isCompleted: true }).populate({
      path: "pledge",
...
const createAward = async (req, res) => {
  try {
    ...
    const award = await SAward.create({ title, description, icon });
...
const getAwards = async (req, res) => {
  try {
    const awards = await SAward.find().sort({ createdAt: 1 });
...
const updateAward = async (req, res) => {
  try {
    const updated = await SAward.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
const deleteAward = async (req, res) => {
  try {
    const deleted = await SAward.findByIdAndDelete(req.params.id);
